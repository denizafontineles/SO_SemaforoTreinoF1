...
public class SemaforoTreinoFormula1 extends Thread{
	
	private int idCarro;
	private int numeroEquipe;
	private Semaphore semaforoCarrosPista;
	private int menorTempo;
	private static int qtdCarrosTerminaram;
	private static int equipes[][] = new int[14][3];

	public SemaforoTreinoFormula1(int numeroEquipe, Semaphore semaforoCarrosPista) {
	...
	public void run() {
		...
		if (qtdCarrosTerminaram == 14) {
			...
			System.out.println();
			System.out.println("# AS EQUIPES TERMINARAM! #");
			System.out.println();
			ranking();
		}
	...
	public void corridaPista(int idCarro) {
		...
		for (int i = 1; i <= 3; i++) {
			int tempo = randomTempo(2000, 500);
			try {
				sleep(tempo);
			} catch (InterruptedException e) {
				e.printStackTrace();
		...
		System.out.println("O carro " + idCarro + " da equipe " + numeroEquipe
				+ " terminou suas 3 voltas e saiu da pista, seu melhor tempo foi de " + menorTempo);
	}

	private void contabilizarMenorTempo() {
		equipes[qtdCarrosTerminaram][0] = idCarro;
		equipes[qtdCarrosTerminaram][1] = numeroEquipe;
		equipes[qtdCarrosTerminaram][2] = menorTempo;
		qtdCarrosTerminaram++;
	}
	...
	public static int randomTempo(int maximo, int minimo) {
